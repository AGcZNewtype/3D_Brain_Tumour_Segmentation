...
这次使用的是UNet模型，是医学深度学习中比较常用的一个，下面是模型的结构和运算过程。

![image-20250803135250429](./assets/image-20250803135250429.png)

在论文的案例中，输入是一个二维的图像，结构为1x572x572，然后通过卷积对特征进行提取，使用ReLU进行激活，然后通过最大池化保留强特征转化成更小的图像，然后再继续进行卷积计算，池化，总共进行4次循环，就能得到一个极小空间的特征图，然后对特征图进行
